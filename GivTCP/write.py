...
def forceExport(export_time):
    ...
    try:
        ...
        payload['finish']=GivLUT.getTime(datetime.now()+timedelta(minutes=export_time))
        payload['slot']=2
        setDischargeSlot(payload)
        payload={}
        ...
        setBatteryMode(payload)
        payload={}
        logger.debug("Max discharge rate for inverter is: " + str(max_discharge_rate))
        ...
        fejob=GivQueue.q.enqueue_in(timedelta(minutes=export_time),FEResume,revert)
...
def tempPauseDischarge(pauseTime):
    ...
    try:
        ...
        if exists(GivLUT.regcache):      # if there is a cache then grab it
            with open(GivLUT.regcache, 'rb') as inp:
                reg_cache_stack= pickle.load(inp)
            revertRate=reg_cache_stack[4]["Control"]["Battery_Discharge_Rate"]
        else:
            revertRate=2600
        ...
        f=open(".tpdRunning", 'w',encoding='ascii')
...
def setBatteryMode(payload):
    ...
    if exists(GivLUT.regcache):      # if there is a cache then grab it
        with open(GivLUT.regcache, 'rb') as inp:
            reg_cache_stack= pickle.load(inp)
    logger.debug("Current battery mode from pickle is: " + str(reg_cache_stack[4]["Control"]["Mode"] ))
    try:
        if payload['mode']=="Eco":
        ...
        error = sys.exc_info()
        temp['result']="Setting Battery Mode failed: " + str(error)
        logger.error (temp['result'])
    return json.dumps(temp)
...
def switchRate(payload):
    ...
    if not isinstance(payload, dict):
        payload=json.loads(payload)
    if GivSettings.dynamic_tariff == False:     # Only allow this control if Dynamic control is enabled
